...
def per_patient_metrics(final_prediction_df):
    ...
    for patient in final_prediction_df.patient.unique():
        ...
        patients_results[patient]['conf_matrix'] = met.confusion_matrix(label,
                                                                        binary_pred)
        patients_results[patient]['accuracy'] = met.accuracy_score(label,
                                                                   binary_pred)
        patients_results[patient]['recall'] = met.recall_score(label,
                                                               binary_pred)
        patients_results[patient]['precision'] = met.precision_score(label,
                                                                     binary_pred)
        try:
...
def global_metrics(final_prediction_df):
    ...
    print('Sensitivity (recall):', round(met.recall_score(label, binary_pred),
                                         3))
...
def per_visit_compare_updrs(small_segments_predictions):
    per_visit = small_segments_predictions[['patient',
                                            'prediction_probability',
                                            'visit_date']].groupby(['patient', 'visit_date']).agg(['mean', 'median'])
