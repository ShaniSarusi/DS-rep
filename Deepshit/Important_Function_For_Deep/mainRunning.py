...
if Prepre_Data == True:    
    execfile(mother_path + 'BeforeDeepLearning.py')
                        
data_for_model_Fourier =  np.stack((verFilter,horFilter ),axis=1)
data_for_model_raw_normlize =  np.stack((normlize_sig(verDenoise),normlize_sig(horDenoise) ),axis=1)
data_for_model_raw =  np.stack((verDenoise,horDenoise ),axis=1)
...
if BuildDeepShit == False: 
    ...
    decoded, encoded =runnetwork(BuildCNNNet2, 10000,data = data_for_model)
    decodedRaw, encodedRaw =runnetwork(BuildCNNNetRaw, 10000,data = data_for_model)
    decodedRawMSE, encodedRawMSE =runnetwork(BuildCNNNetRaw, 10000,data = data_for_model,lost = 'mse')
   
 
Tagdata = np.reshape(np.stack((TagVerFilter, TagHorFilter),axis=1),(np.shape(TagVerFilter)[0],2,np.shape(TagVerFilter)[1],1))
encoded_home = encoded.predict(Tagdata)
Tagdeepfft = np.reshape(encoded_home,(np.shape(Tagdata)[0],64))
Tagdata2 = np.reshape(np.stack((map(normlize_sig,verDenoise_proj),map(normlize_sig,horDenoise_proj)),axis=1),(np.shape(verDenoise_proj)[0],2,np.shape(verDenoise_proj)[1],1))
encoded_home = encodedRaw.predict(Tagdata2)
TagdeepRaw  =  np.reshape(encoded_home,(np.shape(Tagdata2)[0],32))
Tagdata3 = np.reshape(np.stack(((verDenoise_proj),(horDenoise_proj)),axis=1),(np.shape(verDenoise_proj)[0],2,np.shape(verDenoise_proj)[1],1))
encoded_home = encodedRaw.predict(Tagdata3)
TagdeepRawMSE  =  np.reshape(encoded_home,(np.shape(Tagdata3)[0],32))
TagLow = np.column_stack((Tagdeepfft,TagdeepRaw,TagdeepRawMSE))
