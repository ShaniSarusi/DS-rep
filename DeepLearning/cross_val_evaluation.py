...
def make_cross_val(data, Task_params, Task_andSymp, meta_data, deep_task_ids, main_network,  second_network, augment_or_not, deep_params):
    ...
    for train, test in cv:
        ...
        main_network.fit([xtrain, second_network[Mytrain]], [Task_params[Mytrain], Task_params[Mytrain], Task_andSymp[Mytrain]],
                         epochs=deep_params['epochs'] , batch_size=deep_params['batch_size'], shuffle=True, 
                         validation_data=([data[Mytest], second_network[Mytest]], [Task_params[Mytest], Task_params[Mytest], Task_andSymp[Mytest]]),
    ...
    return res, order1, symp_cor_res
...
def make_cross_val_with_auto(data, home_or_not, symp, meta_data, deep_task_ids, main_network,  second_network, augment_or_not, deep_params):
    ...
    for train, test in cv:
        ...
        xtest = data[test]
        xtrain = data[Mytrain]
        main_network.fit([xtrain, home_or_not[Mytrain]], [symp[Mytrain],xtrain],epochs=deep_params['epochs'] , batch_size=deep_params['batch_size'], shuffle=True, validation_data=([xtest[augment_or_not[test] == 1],home_or_not[test][augment_or_not[test] == 1]], [symp[test][augment_or_not[test] == 1],xtest[augment_or_not[test] == 1]]),callbacks=[deep_params['change_lr']] ,verbose=2)#
