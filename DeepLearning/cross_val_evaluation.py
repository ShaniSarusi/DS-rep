...
def make_cross_val(data, task_feature, Task_params, Task_andSymp, meta_data, deep_task_ids, main_network,  second_network, augment_or_not, deep_params):
    ...
    for train, test in cv:
        ...
        temp_res = main_network.predict([xtest[augment_or_not[test] == 1], xtest_features[augment_or_not[test] == 1]])
        features_from_deep.append(second_network.predict([xtest[augment_or_not[test] == 1], xtest_features[augment_or_not[test] == 1]]))
...
def make_cross_val_with_auto(data, home_or_not, symp, meta_data, deep_task_ids, main_network,  second_network, augment_or_not, deep_params):
    ...
    for train, test in cv:
        ...
        main_network.fit([xtrain, home_or_not[train]], [symp[train],xtrain],epochs=deep_params['epochs'] , batch_size=deep_params['batch_size'], shuffle=True,
                         validation_data=([xtest[augment_or_not[test] == 1],home_or_not[test][augment_or_not[test] == 1]], 
                                          ...
                                          callbacks=[deep_params['change_lr']] ,verbose=2)#
        temp_res = main_network.predict([xtest[augment_or_not[test] == 1], np.ones(len(xtest[augment_or_not[test] == 1]))])
        res.append(temp_res[0])
    ...
    return res, order1, features_from_deep, symp_cor_res
