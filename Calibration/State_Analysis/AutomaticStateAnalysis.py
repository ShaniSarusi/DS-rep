...
class AutomaticStateAnalysis:
    ...
    def calc_windows(self, verbose=False):
        ...
        cols = ['SampleId', 'TSstart', 'Duration', 'NumSamples', 'Xstd', 'Ystd', 'Zstd', 'Nstd', 'Xmean', 'Ymean',
                'Zmean', 'Nmean']
    ...
    def find_window_stds_by_duration(self, min_samples, search_intervals=500):
        ...
        self.windows = pd.DataFrame(columns=['Df_data_index', 'TSstart', 'Duration', 'NumSamples', 'Xstd', 'Ystd',
                                             'Zstd', 'Xmean', 'Ymean', 'Zmean'])
        idx_first = 0
        n_win = 0
