...
#######################################################################
clinic_steps_df = pd.read_csv(c.clinic_w_steps_file_path)
inds = pd.isnull(clinic_steps_df).any(1).nonzero()[0]
print('len of inds:', len(inds))
print(clinic_steps_df.user_id.unique())
clinic_steps_df = clinic_steps_df[clinic_steps_df.user_id.isin(valid_users)]
# Convert relevant columns to datetime format:
clinic_steps_df['assessment_start'] = pd.to_datetime(clinic_steps_df['assessment_start'])
clinic_steps_df['assessment_end'] = pd.to_datetime(clinic_steps_df['assessment_end'])
clinic_steps_df['timestamp'] = pd.to_datetime(clinic_steps_df['timestamp'])

sub_clinic_steps_df = clinic_steps_df[['user_id', 'assessment_id', 'assessment_start', 'assessment_end',
                                       'timestamp', 'step_name', 'x', 'y', 'z', 'patient_report_value', 'clinician_report_value']]
sub_clinic_steps_df.to_pickle(join(c.data_path, 'clinic_steps_processed_pickle.p'))


#######################################################################
home_steps_df = pd.read_csv(c.home_w_steps_file_name)
inds = pd.isnull(home_steps_df).any(1).nonzero()[0]
print('len of inds:', len(inds))
print(home_steps_df.user_id.unique())
home_steps_df = home_steps_df[home_steps_df.user_id.isin(valid_users)]
# Rename for compatibility:
home_steps_df['patient_report_value'] = home_steps_df['value']
# Convert relevant columns to datetime format:
home_steps_df['assessment_start'] = pd.to_datetime(home_steps_df['assessment_start'])
home_steps_df['assessment_end'] = pd.to_datetime(home_steps_df['assessment_end'])
home_steps_df['timestamp'] = pd.to_datetime(home_steps_df['timestamp'])
# Rearrange the columns and save:
sub_home_steps_df = home_steps_df[['user_id', 'assessment_id', 'assessment_start', 'assessment_end',
                                   'timestamp', 'step_name', 'x', 'y', 'z', 'patient_report_value']]
sub_home_steps_df.to_pickle(join(c.data_path, 'home_steps_processed_pickle.p'))
